 ...
 */
function getTogglerElement(element) {
  var id = getTogglableId(element);
 ...
 */
function getTogglableElement(toggler) {
 ...
 */
function getTogglableId(element) {
  if (!element.hasAttribute('id')) {
    idMax++;
    element.id = idPrefix + idMax;
  }
  return element.id;
}
...
function toggle(toggler, collapse) {
  var element = getTogglableElement(toggler);
  if (element) {
 ...
 */
function getTogglableParents(element) {
  var elements = [];
...
function handleToggle(event) {
  ...
  if (target) {
    collapseAll(getTogglableParents(target));
    toggle(target);
  }
...
function handleEscape(event) {
  ...
  if (key === 27) {
    ...
    if (!target.hasAttribute('data-cd-toggler')) {
      target = getTogglerElement(target);
    }
 ...
 */
function setTogglable(element) {
  var toggler = getTogglerElement(element) || element.previousElementSibling;
  ...
  toggler.setAttribute('aria-controls', getTogglableId(element));
 ...
 */
function unsetTogglable(element) {
  var toggler = getTogglerElement(element);
 ...
 * handling.
 */
function initializeTogglables() {
  // Retrieve the max ID for generated toggable IDs so we can generate new
  // unique ones.
  // TODO: this isn't selecting any toggables - why not?
  var toggables = document.querySelectorAll("[data-cd-toggable]");
  for (var i = 0, l = toggables.length; i < l; i++) {
    var toggable = toggables[i];
    if (toggable.hasAttribute('id') && toggable.id.indexOf(idPrefix) === 0) {
      var id = parseInt(toggable.id.slice(idPrefix.length - 1), 10);
      if (id > idMax) {
        idMax = id;
      }
    }
...
if (document.documentElement.classList.contains('js')) {
  ...
  window.addEventListener('resize', handleResize);
  var idMax = 0;
  var idPrefix = 'cd-dropdown-toggable-';

  // Initialize toggable menus and listboxes.
  initializeTogglables();
