...
| ------------------ | ----------- | ------ |
| **APIURL**         | Main url. [More information.](#api-url)  | `api.reliefweb.org` |
...
```
http://api.reliefweb.org/v0/report/list?limit=1
```
...
```
curl -XGET 'http://api.reliefweb.org/v0'
```
...
```json
{
	...
	"data": {
		...
		"endpoints": {
			"report": {
				"info": "http:\/\/api.reliefweb.org\/v0\/report\/info",
				"list": "http:\/\/api.reliefweb.org\/v0\/report\/list",
				"item": "http:\/\/api.reliefweb.org\/v0\/report\/[ID]"
			},
			"job": {
				"info": "http:\/\/api.reliefweb.org\/v0\/job\/info",
				"list": "http:\/\/api.reliefweb.org\/v0\/job\/list",
				"item": "http:\/\/api.reliefweb.org\/v0\/job\/[ID]"
			},
			"training": {
				"info": "http:\/\/api.reliefweb.org\/v0\/training\/info",
				"list": "http:\/\/api.reliefweb.org\/v0\/training\/list",
				"item": "http:\/\/api.reliefweb.org\/v0\/training\/[ID]"
			},
			"country": {
				"info": "http:\/\/api.reliefweb.org\/v0\/country\/info",
				"list": "http:\/\/api.reliefweb.org\/v0\/country\/list",
				"item": "http:\/\/api.reliefweb.org\/v0\/country\/[ID]"
			},
			"disaster": {
				"info": "http:\/\/api.reliefweb.org\/v0\/disaster\/info",
				"list": "http:\/\/api.reliefweb.org\/v0\/disaster\/list",
				"item": "http:\/\/api.reliefweb.org\/v0\/disaster\/[ID]"
			}
		}
	}
}
...
```json
curl -XGET 'http://api.reliefweb.org/v0/report/list' -d '{
	"query": {
		"fields": ["title", "body"],
		"value": "humanitarian"
	}
...
```
http://api.reliefweb.org/v0/report/list?query[value]=humanitarian&query[fields][0]=title&query[fields][1]=body
```
...
```
curl -XGET 'http://api.reliefweb.org/v0/report'
```
...
```
curl -XGET 'http://api.reliefweb.org/v0/country/info'
```
...
```json
curl -XGET 'http://api.reliefweb.org/v0/report/count' -d '{
  "query": {
    "value" : "country:Japan"
  }
}'
...
```
curl -XGET 'http://api.reliefweb.org/v0/report/573658'
```
...
```json
curl -XGET 'http://api.reliefweb.org/v0/report/573658' -d '{
  "fields": {
...
```
http://api.reliefweb.org/v0/report/list?limit=5&fields[include][0]=url&fields[include][1]=primary_country.name&fields[include][2]=title&filter[field]=headline&sort[0]=date.created:desc
```
...
```
http://api.reliefweb.org/v0/report/list?limit=1&fields[include][0]=url&fields[include][1]=title&fields[include][2]=file.preview&query[value]=primary_country:Syria format:map&filter[field]=file.preview&sort[0]=date.created:desc
```
...
```
http://api.reliefweb.org/v0/report/list?limit=1&fields=country
```
