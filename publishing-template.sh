...
if [ -n "$embargoed" ]; then
  data+=", \"embargoed\": ${embargoed}"
fi
if [ -n "$origin" ]; then
  data+=", \"origin\": ${origin}"
fi
if [ -n "$file_url" ]; then
  data+=", \"file\": [ { \"url\": $file_url, \"uuid\": ${file_uuid}, \"filename\"${file_filename}, \"checksum\": $file_checksum, \"file_language\": ${file_language:-en} } ]"
fi
if [ -n "$image" ]; then
  data+=", \"image\": [ { \"url\": $image_url, \"uuid\": ${image_uuid}, \"checksum\": $image_checksum, \"description\"${image_description}, \"copyright\": ${image_copyright} } ]"
fi
if [ -n "$disasters" ]; then
  data+=", \"disaster\": ${disasters}"
fi
